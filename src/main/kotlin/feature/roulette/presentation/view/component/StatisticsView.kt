...
class StatisticsView : RoundedPanel(15) {
    ...
    fun updateStatistics(successful: Int, failed: Int) {
        ...
        if (total == 0) {
            ...
            val probability = (successful.toDouble() / total.toDouble())
            println(probability.formatToScientific())
            probabilityLabel.text = "Success Probability: ${probability.formatToScientific()}"
    ...
    fun updateTheoreticalProbability(probability: Double?) {
        if (probability == null) {
            ...
            val percentage = probability
            theoreticalProbabilityLabel.text = "Theoretical Probability: ${percentage.formatToScientific()}"
        }
    }

    private fun Double.formatToScientific(): String {
        if (this == 0.0) return "0.0"

        // Use BigDecimal to get a precise string representation without default rounding.
        ...
        // Create the mantissa (the significant digits)
        val mantissa = plainString.replace(".", "").trimStart('0')
        val formattedMantissa = "${mantissa.first()}.${mantissa.substring(1)}"
